...
export default function BookingWizard({ initialCatalog }: { initialCatalog?: InitialResource }) {
  ...
  return <div className="wizardShell"><div className="stepper" aria-label="Booking progress">{steps.map((s, i) => <div className={`step ${i <= step ? 'active' : ''}`} key={s} aria-current={i === step ? 'step' : undefined}><span>{i + 1}</span><em>{s}</em></div>)}</div><div className="wizardPanel"><ErrorNotice message={error}/>
